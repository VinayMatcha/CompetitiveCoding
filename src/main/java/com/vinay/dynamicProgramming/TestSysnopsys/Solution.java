package com.vinay.dynamicProgramming.TestSysnopsys;

// You may import additional classes here if needed.
import java.util.*;
...
public class Solution {
    ...
    public Set<Project> findProjectsRequiringUpdates(Set<Project> allProjects, Set<Update> updates) {
          HashMap<Integer, Component> componentsupdated = new HashMap<>();
          Set<Project> result = new HashSet<>();
          Iterator<Update> it = updates.iterator();
          while ( it.hasNext() ) {
              Update update = it.next();
              Set<Component> set = update.components;
              Iterator<Component> it1 = set.iterator();
              while ( it1.hasNext() ) {
                  Component component = it1.next();
                  componentsupdated.put(component.id, component);
              }
          }
          Iterator<Project> pro =  allProjects.iterator();
          while (pro.hasNext()) {
              Project project = pro.next();
              Set<Component> components = project.getComponents();
              Iterator<Component> it1 = components.iterator();
              while ( it1.hasNext() ) {
                  Component component = it1.next();
                  if (componentsupdated.containsKey(component.id) ) {
                      Component componentCheck = componentsupdated.get(component.id);
    ...
    public String solution(String S) {
        ...
        TestCase t = Solution.makeTestCase(S);
        ...
        return Solution.projectSetToString(results);
    }
